locals {
  ...
  state-file-name = "terraform-${local.env}.tfstate"
  path = "${local.env}/${local.state-file-name}"
}
module "label" {
...
resource "aws_s3_bucket" "fiap" {
  ...
  lifecycle_rule {
    ...
    dynamic "noncurrent_version_transition" {
      for_each = var.enable_glacier_transition ? [
        1] : []
...
data "aws_iam_policy_document" "bucket_policy" {
  ...
  statement {
    ...
    actions = [
      "s3:PutObject"]
    resources = [
      "arn:aws:s3:::${join("", aws_s3_bucket.fiap.*.id)}/*"]
  ...
  statement {
    ...
    actions = [
      "s3:PutObject"]
    resources = [
      "arn:aws:s3:::${aws_s3_bucket.fiap[0].id}/*"]
...
resource "aws_s3_bucket_policy" "default" {
  count = var.enabled && var.allow_encrypted_uploads_only ? 1 : 0
  bucket = join("", aws_s3_bucket.fiap.*.id)
...
resource "aws_s3_bucket_object" "upload" {
  bucket = join("", aws_s3_bucket.fiap.*.id)
  key = local.path
  source = "./terraform.tfstate.d/${local.env}/terraform.tfstate"
}
